    ...
    diffDate: function(d, more = false) {
        ...
        if (more) {
            ...
            if (monthCount >= 1) {
                ...
                result = parseInt(dayCount) + " " + (window.GLOBAL_CONFIG?.date_suffix?.day || "天前");
            } else if (hourCount >= 1) {
                result = parseInt(hourCount) + " " + (window.GLOBAL_CONFIG?.date_suffix?.hour || "小时前");
            } else if (minuteCount >= 1) {
                result = parseInt(minuteCount) + " " + (window.GLOBAL_CONFIG?.date_suffix?.min || "分钟前");
            } else {
                result = window.GLOBAL_CONFIG?.date_suffix?.just || "刚刚";
            }
        } else {
...
// ==================== 复制提醒 ====================
function setupCopyNotification() {
...
function essayScroll() {
    ...
    let result = waterfallResult <= 99 ? waterfallResult : 99;
    ...
    if (p && (p.offsetTop + p.offsetHeight / 2 < r || 90 < result)) {
        percentFlag = true;
    }
}
...
function getRandom(option) {
    ...
    switch (option) {
        case 'x': return random * window.innerWidth;
        case 'y': return random * window.innerHeight;
        case 's': return random;
        case 'r': return random * 6;
        case 'fnx':
            const fnxRandom = -0.4 + random * 0.8;
            return (x, y) => x + 0.2 * fnxRandom - 0.75;
        case 'fny':
            const fnyRandom = 0.4 + random * 0.2;
            return (x, y) => y + fnyRandom;
        case 'fnr':
            const fnrRandom = random * 0.03;
            return (r) => r + fnrRandom;
        default: return 0;
    }
}
...
class SakuraController {
    ...
    setupCanvas() {
        ...
        this.canvas.setAttribute('style', 'position: fixed;left: 0;top: 0;pointer-events: none;');
