...
function getRandom(option) {
    ...
    switch (option) {
        ...
        case 'r':
            ...
            break;
        case 'fnx':
            random = -0.4 + Math.random() * 0.8;
            ret = function (x, y) {
                return x + 0.2 * random - 0.75;
            };
            break;
        case 'fny':
            random = 0.4 + Math.random() * 0.2
            ret = function (x, y) {
                return y + random;
            };
            break;
...
img.onload = function () {
    // 当页面路径是主页“/” 或者 是 “/about/” 页面时，启动樱花效果
    if (window.location.pathname === '/' || window.location.pathname === '/about/' || window.location.pathname === '/link/' || window.location.pathname === '/dcim/' || window.location.pathname === '/bt/') {
        startSakura();
    } else {
        // 如果是其他页面，则移除画布并停止动画，防止效果残留
